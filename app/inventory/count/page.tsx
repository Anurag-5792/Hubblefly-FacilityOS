    ...
    <main className="workflow-page">
      ...
      <form className="panel workflow-form" onSubmit={submit}>
        <div className="workflow-grid">
          <label>
            <span>Count target QR / ID</span>
            <input value={target} onChange={(e) => setTarget(e.target.value)} placeholder="Serial, batch, position or container" />
            <small>{target ? `${targetResolved.type.toUpperCase()} · ${targetResolved.title}` : 'Scan or type target'}</small>
          </label>
          <label>
          ...
          <label>
            <span>Container (optional)</span>
            <input value={container} onChange={(e) => setContainer(e.target.value)} placeholder="BN / BX / BB" />
            <small>{container ? `${containerResolved.type.toUpperCase()} · ${containerResolved.title}` : 'Optional physical container'}</small>
          </label>
          <label>
            <span>Condition</span>
        ...
        <div className="workflow-actions">
          <button className="scan-primary" type="submit" disabled={busy}>{busy ? 'Checking…' : 'Validate Count'}</button>
          <span>Validation only — no ERPNext adjustment is submitted.</span>
        </div>
      </form>
      ...
      {preview && (
        <section className={preview.ok ? 'panel workflow-result success' : 'panel workflow-result error'}>
          ...
          <h2>{preview.ok ? 'Count is ready for verification' : 'Count needs correction'}</h2>
          ...
          {preview.ok && (
            <dl className="result-fields">
              ...
              <div><dt>Control note</dt><dd>{preview.note}</dd></div>
            </dl>
          )}
        </section>
      )}
