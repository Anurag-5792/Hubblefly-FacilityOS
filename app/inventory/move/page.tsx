    ...
    <main className="workflow-page">
      ...
      <form className="panel workflow-form" onSubmit={submit}>
        <div className="workflow-grid">
          <label>
            <span>Source QR / ID</span>
            <input value={source} onChange={(e) => setSource(e.target.value)} placeholder="Serial, batch or container" />
            <small>{source ? `${sourceResolved.type.toUpperCase()} · ${sourceResolved.title}` : 'Scan or type source'}</small>
          </label>
          <label>
            <span>Destination QR / ID</span>
            <input value={destination} onChange={(e) => setDestination(e.target.value)} placeholder="Position or container" />
            <small>{destination ? `${destinationResolved.type.toUpperCase()} · ${destinationResolved.title}` : 'Scan or type destination'}</small>
          </label>
          <label>
            <span>Quantity</span>
        ...
        <div className="workflow-actions">
          <button className="scan-primary" type="submit" disabled={busy}>{busy ? 'Checking…' : 'Validate Move'}</button>
          <span>No stock is posted at this stage.</span>
        </div>
      </form>
      ...
      {preview && (
        <section className={preview.ok ? 'panel workflow-result success' : 'panel workflow-result error'}>
          ...
          <h2>{preview.ok ? 'Move is structurally valid' : 'Move needs correction'}</h2>
          ...
          {preview.ok && (
            <dl className="result-fields">
              ...
              <div><dt>Future ERPNext action</dt><dd>{preview.erpNextTransaction}</dd></div>
            </dl>
          )}
        </section>
      )}
