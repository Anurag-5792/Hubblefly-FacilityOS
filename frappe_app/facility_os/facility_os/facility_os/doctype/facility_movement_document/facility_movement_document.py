...
class FacilityMovementDocument(Document):
    def before_insert(self):
        ...
        self.erp_reconciliation_status = self.erp_reconciliation_status or "Not Linked"
    ...
    def validate(self):
        ...
        if self.document_type == "GRN":
            ...
            if not self.recipient or not self.purpose:
                frappe.throw("Outward movement requires Recipient and Purpose.")

        if self.document_type == "Outward Movement":
            if not self.generate_gate_pass and not self.generate_delivery_challan:
                frappe.throw("Select at least one output: Gate Pass or Delivery Challan.")
            if self.returnable and not self.expected_return_date:
                frappe.throw("Expected Return Date is required for returnable outward material.")
